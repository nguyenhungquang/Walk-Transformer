...
class SANNE(nn.Module):
    ...
    def __init__(self, vocab_size, rel_size,feature_dim_size, ff_hidden_size, sampled_num,
        ...
        self.ss = SampledSoftmax(self.vocab_size, self.rel_size, self.sampled_num, self.feature_dim_size, self.device)
    ...
    def hit_at_10(self, input_x,input_r,input_t):
        ...
        #relations
        r_embed=self.rel_feature(torch.tensor([input_r]))
        r_sign=torch.sign(torch.tensor([input_r]))
        r_embed*=r_sign[:,None]
        r_embed = F.normalize(r_embed, p=2, dim=-1).detach().numpy()
        #score
