...
def hitat10(embedding, w, test, batch_size=1,model=None):
    ...
    for idx in iterator:
        ...
        batch_a=batch_a.repeat_interleave(num_ent).unsqueeze(1)
...
def perturb_and_get_raw_rank(embedding, w, a, r, b, test_size, batch_size=100,model=None):
    """ Perturb one element in the triplets
    """
    n_batch = (test_size + batch_size - 1) // batch_size
    ranks = []
    ...
    for idx in range(n_batch):
        ...
        batch_input=torch.cat([batch_a,batch_r,batch_ents],dim=1)
        score=model.calc_score(embedding,batch_input)
        score=score.reshape(b_size,num_ent)
        target = b[batch_start: batch_end]
        ranks.append(sort_and_rank(score, target))
        # print(embedding.shape)
            # conv_input = torch.cat([batch_a, batch_r, batch_b], 1)
...
def calc_raw_mrr(embedding, w, test_triplets, hits=[], eval_bz=100,model=None):
    with torch.no_grad():
        s = test_triplets[:, 0]
        r = test_triplets[:, 1]
        o = test_triplets[:, 2]
        ...
        # perturb subject
        ranks_s = perturb_and_get_raw_rank(embedding, w, o, r, s, test_size, eval_bz,model=model)
        # perturb object
        ranks_o = perturb_and_get_raw_rank(embedding, w, s, r, o, test_size, eval_bz,model=model)
...
def calc_mrr(embedding, w, train_triplets, valid_triplets, test_triplets, hits=[], eval_bz=100, eval_p="filtered",model=None):
    if eval_p == "filtered":
        mrr = calc_filtered_mrr(embedding, w, train_triplets, valid_triplets, test_triplets, hits)
    else:
        mrr = calc_raw_mrr(embedding, w, test_triplets, hits, eval_bz,model)
    return mrr
